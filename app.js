...
// Load sample data (these JSON files are included in /data). We'll attempt fetch; if unavailable (e.g., GitHub pages still serves), fallback to embedded samples.
...
const SAMPLE_NEWS = await (async()=>{
  return await loadJSON('data/news.json', [
...
const SAMPLE_GALLERY = await (async()=>{
...
const SAMPLE_VIDEOS = await (async()=>{
...
function writeStored(key,data){ localStorage.setItem(key, JSON.stringify(data)); }

let NEWS = [...SAMPLE_NEWS, ...readStored(STORAGE_KEYS.news,[])];
let GALLERY = [...SAMPLE_GALLERY, ...readStored(STORAGE_KEYS.gallery,[])];
let VIDEOS = [...SAMPLE_VIDEOS, ...readStored(STORAGE_KEYS.videos,[])];
...
function renderHome(){
  ...
  document.getElementById('newsForm').addEventListener('submit', e=>{
    ...
    NEWS.push(item); writeStored(STORAGE_KEYS.news, NEWS.filter(n=>n.id));
    alert('Published locally — it will show up for visitors of this device. To publish globally, add your news.json file to repo/data.');
    location.hash = '#news';
  });
  ...
  document.getElementById('imgForm').addEventListener('submit', async e=>{
    ...
    GALLERY.push(item); writeStored(STORAGE_KEYS.gallery,GALLERY);
  ...
  document.getElementById('videoForm').addEventListener('submit', async e=>{
    ...
    VIDEOS.push(item); writeStored(STORAGE_KEYS.videos,VIDEOS);
...
function renderNewsList(){
  ...
  NEWS.slice().reverse().forEach(n=>{
    const el = document.createElement('article'); el.setAttribute('role','listitem'); el.className='card';
    el.innerHTML = `<h3><a href="#news/${n.id}">${escapeHtml(n.title)}</a></h3><p class="meta">${n.date} • ${n.author||'Community'}</p><p>${n.excerpt||excerptFrom(n.content)||''}</p>`;
    grid.appendChild(el);
  });
}
...
function renderGallery(){
  ...
  GALLERY.slice().reverse().forEach(it=>{
    const el = document.createElement('figure'); el.className='card'; el.setAttribute('role','listitem');
    if(it.type==='image'){
...
function excerptFrom(html){ return (html||'').replace(/<[^>]*>/g,'').slice(0,200)+'...'; }
function fileToDataURL(file){ return new Promise((res,rej)=>{ const fr=new FileReader(); fr.onload=()=>res(fr.result); fr.onerror=rej; fr.readAsDataURL(file); }); }

// Support deep links like #news/:id
navigate();
